...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
    ...
    const menuItems = [
        ...
        {
            label: 'Online Users', href: '/discover', icon: (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
            ...
            <div className={`fixed top-0 right-0 h-full w-80 bg-slate-900/95 backdrop-blur-xl z-[101] shadow-2xl border-l border-white/10 transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-8 flex flex-col items-center">
                        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-rose-500 to-amber-500 flex items-center justify-center text-2xl font-bold text-white shadow-lg mb-4 ring-4 ring-rose-500/20 overflow-hidden">
                            {session?.user?.image ? (
                        ...
                        <h3 className="text-lg font-bold text-white">{session?.user?.name || 'Tether User'}</h3>
